...
export default function AnalysisPanel({
  ...
  const hasUsableScore = typeof analysis?.overallScore === 'number' && analysis.overallScore > 0;
  const hasSummary = typeof analysis?.summary === 'string' && analysis.summary.trim().length > 0;
  const shouldShowSuccess = Boolean(analysis && hasUsableScore && hasSummary && !hasErrorKeywords);
  ...
  if (!shouldShowSuccess) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-12 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">等待分析结果</h3>
        <p className="text-slate-500 dark:text-slate-400">分析结果正在同步，请稍后自动刷新</p>
      </div>
    );
  }
          ...
          <div
            ...
            {analysis.strengths && analysis.strengths.length > 0 && (
                ...
                <div className="flex flex-wrap gap-2">
                  {analysis.strengths.map((s: { category?: string; description?: string }, i: number) => (
                      <span key={i}
