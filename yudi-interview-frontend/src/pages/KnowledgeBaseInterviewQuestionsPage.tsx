...
export default function KnowledgeBaseInterviewQuestionsPage() {
  ...
  const handleCancelGeneration = async () => {
    ...
    setActionError('');
    try {
      const result = await knowledgeBaseApi.cancelQuestionGeneration(knowledgeBaseIdNum);
      setGenerationStatus(result);
      // 取消时可能已有部分题目落库，刷新题目列表与知识库信息
      await loadQuestions();
      void loadKnowledgeBase();
    } catch (error) {
  ...
  const retryConfig = generationStatus?.questionGenConfig ?? null;
        ...
        <div className="flex gap-2 shrink-0">
          ...
          {generationActive && (
            <button
              ...
              disabled={cancelling}
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-red-200 dark:border-red-900 text-red-600 dark:text-red-400 text-sm font-medium hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
            >
              {cancelling
                ? <Loader2 className="w-4 h-4 animate-spin" />
        ...
        {generationNotice && (
          <div className={`flex items-center justify-between gap-3 rounded-lg border px-4 py-3 text-sm ${
            ...
            {(generationStatus?.questionGenStatus === 'FAILED'
              || generationStatus?.questionGenStatus === 'CANCELLED') && (
              <button
                type="button"
                onClick={() => {
                  setGenerateError('');
