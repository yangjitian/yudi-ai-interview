...
export default function KnowledgeBaseManagePage({ onUpload, onChat }: KnowledgeBaseManagePageProps) {
  ...
  const loadDataSilent = useCallback(async () => {
    // 列表独立加载，失败不影响页面
    try {
      const kbList = searchKeyword
        ? await knowledgeBaseApi.search(searchKeyword)
        : selectedCategory
        ? await knowledgeBaseApi.getByCategory(selectedCategory)
        : await knowledgeBaseApi.getAllKnowledgeBases(sortBy);
      setKnowledgeBases(kbList);
    } catch (error) {
      console.error('加载知识库列表失败:', error);
    }
    // stats 和 categories 独立加载，失败不影响列表渲染
    try {
      const [statsData, categoryList] = await Promise.all([
