...
import {useEvaluatingSessionsSync} from '../hooks/useEvaluatingSessionsSync';
...
function isEvaluateCompleted(item: UnifiedInterviewItem): boolean {
  if (item.evaluateStatus === 'COMPLETED' || item.evaluateStatus === 'COMPLETED_WITH_ERRORS') return true;
...
export default function InterviewHistoryPage({
  ...
  const loadAll = useCallback(async (isPolling = false) => {
    ...
    try {
      const skillsPromise = skillsLoadedRef.current
        ? Promise.resolve(skillsRef.current)
      ...
      const all = [
        ...mapTextInterviews(textSessions, loadedSkills),
        ...mapVoiceInterviews(voiceSessions, loadedSkills),
      ];
      ...
      if (!isPolling) setError('加载失败，请刷新重试');
    } finally {
  ...
  function mapVoiceInterviews(sessions: SessionMeta[], skills: SkillDTO[]): UnifiedInterviewItem[] {
    return sessions.map(session => ({
      id: `voice-${session.sessionId}`,
      type: 'voice' as const,
      title: getTemplateName(session.roleType, skills),
      sessionId: session.sessionId,
      status: session.status,
  ...
  // SSE 实时同步：监听评估中会话的状态变化
  const handleStatusChange = useCallback(
    (sessionId: string, newStatus: string, overallScore?: number) => {
      setItems(prev =>
        prev.map(item => {
          ...
          return {
            ...
            ...((newStatus === 'COMPLETED' || newStatus === 'COMPLETED_WITH_ERRORS') && overallScore !== undefined
              ? { overallScore }
              : {}),
          };
        }),
  ...
  useEvaluatingSessionsSync(items, handleStatusChange);
