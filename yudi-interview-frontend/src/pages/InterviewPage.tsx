...
export default function Interview({
  ...
  useEffect(() => {
    if (!isCreating || sessionIdToResume) return;
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => sessionIdToResume ? resumeExistingSession(sessionIdToResume) : startInterview()}
      ...
      >
        {session.isFallback && (
          <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
            题目服务暂时不可用，当前使用降级题目。{session.fallbackReason ? `原因：${session.fallbackReason}` : ''}
          </div>
        )}
        <InterviewChatPanel
          session={session}
