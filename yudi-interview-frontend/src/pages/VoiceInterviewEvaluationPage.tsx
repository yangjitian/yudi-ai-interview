...
export default function VoiceInterviewEvaluationPage() {
  ...
  const handleStatusResponse = useCallback((response: EvaluationStatusResponse) => {
    ...
    if ((status === 'COMPLETED' || status === 'COMPLETED_WITH_ERRORS') && response.evaluation) {
  ...
  const loadEvaluation = async (showLoading = true) => {
    ...
    try {
      const status = await voiceInterviewApi.getEvaluation(sessionId);
      handleStatusResponse(status);
    } catch {
      try {
        const status = await voiceInterviewApi.generateEvaluation(sessionId);
        handleStatusResponse(status);
      } catch (err) {
  ...
  if (!evaluation || !interviewDetail || !sessionId) {
    return null;
  }
