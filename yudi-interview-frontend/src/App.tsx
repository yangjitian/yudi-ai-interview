...
import { historyApi, type EvaluateStatus, type InterviewDetail } from './api/history';
import { reEvaluateSession, subscribeEvaluationEvents } from './api/interview';
...
function InterviewDetailPageWrapper() {
  ...
  useEffect(() => {
    ...
    const evaluateStatus = interview.evaluateStatus as string | undefined;
    const isEvaluating = evaluateStatus === 'PENDING' ||
    ...
    const cleanup = subscribeEvaluationEvents(
      ...
      (status) => {
        if (!mounted) return;
        setInterviewRef.current(prev => prev ? {
          ...prev,
          evaluateStatus: status as EvaluateStatus,
          evaluateError: undefined,
        } : prev);
      },
      async (_score) => {
        try {
          const completedDetail = await historyApi.getInterviewDetail(sessionId);
          setInterviewRef.current(completedDetail);
        } catch {
          setInterviewRef.current(prev => prev ? {
            ...prev,
            evaluateError: '加载评估结果失败，请手动刷新',
          } : prev);
        }
      },
      (message) => {
  ...
  }, [sessionId, interview?.evaluateStatus]);
  ...
  const handleReEvaluate = async () => {
    ...
    try {
      ...
      setInterview(prev => prev ? {
        ...prev,
        evaluateError: requestError instanceof Error ? requestError.message : '重新评估失败',
      } : prev);
    } finally {
  ...
  const currentEvaluateStatus = interview?.evaluateStatus as string | undefined;
  const isEvaluating = currentEvaluateStatus === 'PENDING' ||
    currentEvaluateStatus === 'PROCESSING';
  const showReEvaluate = interview && (
    interview.evaluateStatus === 'FAILED' ||
    interview.evaluateStatus === 'COMPLETED' ||
    interview.evaluateStatus === 'COMPLETED_WITH_ERRORS' ||
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        {showReEvaluate && (
          <button
            ...
            disabled={reEvaluating || isEvaluating}
            className="ml-auto px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {reEvaluating || isEvaluating ? '重新评估中...' : '重新评估'}
          </button>
        )}
      </div>
      {interview.evaluateError && (
        <div className="mb-6 rounded-xl border border-red-200 bg-red-50 px-5 py-4 text-red-600 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-400">
          {interview.evaluateError}
        </div>
      )}
      {isEvaluating ? (
        <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl bg-white shadow-sm dark:bg-slate-800">
          <Loader2 className="mb-4 h-8 w-8 animate-spin text-primary-500" />
          <p className="text-slate-600 dark:text-slate-300">AI 正在生成评估报告...</p>
